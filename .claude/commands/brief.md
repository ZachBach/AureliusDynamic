...
> Standalone install (no CONNECTORS.md): before scanning, check which data sources are actually connected — MCP connectors that are not authorized (e.g. Gmail, Google Calendar via claude.ai connector settings) must be listed under "Sources Not Available" rather than silently skipped.
