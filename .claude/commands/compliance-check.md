...
> Standalone install (no CONNECTORS.md): before scanning, check which data sources are actually connected — MCP connectors that are not authorized (e.g. Gmail via claude.ai connector settings) must be listed as unavailable rather than silently skipped.
